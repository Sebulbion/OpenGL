...
ssAnimatedModel::ssAnimatedModel(std::string modelFilname, std::string texFilename, GLuint _program, GLuint _shadowProgram) {
...
void ssAnimatedModel::setShaderEffectVariables() {
	...
	GLint lightVpLoc = glGetUniformLocation(program, "lightVp");
	glUniformMatrix4fv(lightVpLoc, 1, GL_FALSE, glm::value_ptr(lightVp));

	GLint gShadowMapSampler = glGetUniformLocation(program, "shadowMap");
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, CShadowMap::depthMapTexture);
	glUniform1i(gShadowMapSampler, 1);
	...
	glUniform3f(cameraPosLoc, CCamera::GetPosition().x, CCamera::GetPosition().y, CCamera::GetPosition().z);

	glm::vec3 lightPos = CShadowMap::lightPosition;
...
void ssAnimatedModel::setShadowShaderEffectVariables()
{
	glUseProgram(this->shadowProgram);
	...
	glm::mat4 vp = CCamera::getProjectionMatrix() * CShadowMap::lightViewMatrix;

	GLint vpLoc = glGetUniformLocation(shadowProgram, "vp");
	glUniformMatrix4fv(vpLoc, 1, GL_FALSE, glm::value_ptr(vp));

	GLint modelLoc = glGetUniformLocation(shadowProgram, "model");
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void ssAnimatedModel::render(bool _isShadowPass) {

	if(_isShadowPass)
		setShadowShaderEffectVariables();
	else
...
void ssAnimatedModel::boneTransforms(std::vector<Matrix4f>& transforms) {
	...
	// Bad fix to how the jump animation would loop
	if (animationTime >= 2.66f) {
		animationTime = 2.6f;
	}
